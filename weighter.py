...
def make_weighted_corpus(testmode = False):
    ...
    def determine_new_weighting(old_weights, latest_model_best):
        ...
        # first run is defaults
        if first_run:
            return old_weights
        # if everything but root is unmodified
        if all([v == -1 for k, v in old_weights.items()[2:]]) and \
            old_weights['nsubj'] == 1:
            if latest_model_best:
                # either increment root
                new_weights['root'] = new_weights['root'] + 1
                return new_weights
            else:
                # or deincrement root and make nsubj == 2
                new_weights['root'] = new_weights['root'] - 1
                new_weights['nsubj'] = new_weights['nsubj'] + 1
                return new_weights
        #else:
        last_modified = [k for k, v in old_weights.items() if v != -1][-1]
        if latest_model_best:
